...
public class Notepad extends JFrame implements ActionListener {
	...
	public Notepad() {
		...
		this.setTitle("ANKS ka NOTE"); // set the title of the window
		setDefaultCloseOperation(EXIT_ON_CLOSE); // set the default close operation (exit when it gets closed)
		this.textArea.setFont(new Font("Century Gothic", Font.BOLD, 15)); // set a default font for the TextArea
		...
		this.file.setLabel("File operations");
			...
			if (option == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					Scanner scan = new Scanner(new FileReader(open.getSelectedFile().getPath()));
                                        sentText="";
                                        while(scan.hasNext()){
                                            sentText+=(scan.nextLine()+"\n");
                                        }
                                        gotText=Transform.decrypt(sentText);
                                        this.textArea.append(gotText);
                                        /*
		...
		// and lastly, if the source of the event was the "save" option
			...
			if (option == JFileChooser.APPROVE_OPTION) {
				try {
					...
					gotText=Transform.encrypt(this.textArea.getText());
                                        out.write(gotText);
                                        /*
